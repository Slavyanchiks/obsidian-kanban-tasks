...
Плагин для быстрого создания задач в Kanban досках через удобную форму, вызываемую горячими клавишами. Какие ещё задачи в Kanban? Читай [здесь](./PHILOSOPHY.md)
...
#### Настройка групп тегов

В настройки Kanban доски следует добавить информацию о тегах, как поле с ключём `tag_groups` на том же уровне, что и `tag_colors`
```json
"tag_groups" : [
  {
    "name" : "Название группы тегов",
    "keys" : ["#названия тегов с решёткой"]
  }, { ... }
]
```
